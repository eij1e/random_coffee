...
categories = {
    "Пол": ["мужской", "женский"],
    "Пол партнера": ["мужской", "женский", "не важно"],
    "Специализация": [
...
def select_option(call):
    ...
    if category in ["Пол", "Пол партнера"]:
...
def next_category(call):
    ...
    if current_category == "Пол" and "Пол" not in user_choices[chat_id]:
        ...
        return
    if current_category == "Пол партнера" and "Пол партнера" not in user_choices[chat_id]:
        bot.answer_callback_query(call.id, "Пожалуйста, обязательно выберите пол партнера.")
        return
...
def callback_handler(callback):
    ...
    if callback_data == 'message':
        bot.send_message(callback.message.chat.id, 'словарь отправлен в консоль')
        start_command(callback.message)
        ...
        if participant == 'True':
            ...
            if result is None:
                markup.add(types.InlineKeyboardButton('Меню', callback_data='menu'))
                bot.send_message(chat_id=callback.message.chat.id, text='Заполните анкету, чтобы принять участие', parse_mode='html', reply_markup=markup)
                return None
        ...
        markup.add(types.InlineKeyboardButton('Message', callback_data='message'))
