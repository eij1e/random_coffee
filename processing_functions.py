...
import os
import messages
from main import bot, categories
...
def ask_question(message_id, user_id, pair_id, matching_round, question, current_question):
    ...
    if message_id is None:
        message = bot.send_message(user_id, "Вопрос №{}: {}".format(current_question + 1, question), reply_markup=markup)
        ...
        bot.edit_message_text(chat_id=user_id, message_id=message_id,
                              text="Вопрос №{}: {}".format(current_question + 1, question), reply_markup=markup)
        return message_id
    elif current_question == 4:
        bot.edit_message_text(chat_id=user_id, message_id=message_id,
                              text="Ваши ответы сохранены, спасибо за обратную связь !")
    else:
        bot.edit_message_text(chat_id=user_id, message_id=message_id, text="Вопрос №{}: {}".format(current_question + 1, question), reply_markup=markup)
        return message_id


def sent_messages(matches_df, users_file='all_users.csv'):
    possible_questions = [
        'Насколько вас устроил возраст партнера?',
...
def remove_participant(chat_id, file_path='participants.csv'):
    ...
    if str(chat_id) not in df['chat_id'].values:
        return "Пользователь не найден."

    df = df[df['chat_id'] != str(chat_id)]
...
def design_menue(chat_id):
    ...
    if str(chat_id) == '1140637373':  # admin id
        markup.add(types.InlineKeyboardButton('do_matching', callback_data='start_matching'))
...
def save_user_choices(chat_id, choices):
    ...
    if 'Пол партнера' in new_data:
        new_data['pair_sex'] = new_data.pop('Пол партнера')[0]
    if 'Специализация' in new_data:
        new_data['specialization'] = new_data.pop('Специализация')[0]
    if 'age' in new_data:
        new_data['age'] = new_data.pop('age')
