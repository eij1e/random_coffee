...
def read_users(file_path='all_users.csv'):
    """Чтение файла с характеристиками пользователей."""
    try:
...
def find_best_partner(user_id, user_ids, used_pairs, users_df, tag_weights):
    ...
    for candidate_id in user_ids:
        if candidate_id != user_id and tuple(sorted([str(user_id), str(candidate_id)])) not in used_pairs:
...
def perform_matching(participants_file='participants.csv', users_file='all_users.csv', output_file='matches.csv'):
    ...
    if not match_df.empty:
        ...
        complete_data.drop_duplicates(subset=['User 1', 'User 2'], keep='last', inplace=True)
